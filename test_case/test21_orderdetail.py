...
class shanghutong(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 获取ordersn
        ordersn = config.get('parameters', 'ordersn')
        # 获取url
        url = config.get('parameters', 'url')
        self.base_url = url + '/order/detail'
        self.payload = {'token': token, 'userId': user_id, 'orderSn': ordersn}

    def tearDown(self):
        pass

    def test21_orderlist(self):
